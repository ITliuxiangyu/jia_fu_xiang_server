...
def createQrcodeByString(qrcodeStr):
    qr = QRCode()
    qrcodeStr = 'http://www.dangsh.top/static/vue-shop/dist/index.html?wxid=' + qrcodeStr
...
def orderRows_to_orderData(rows):
    ...
    for row in rows:
        ...
        tempDic["createTime"] = row[2].strftime('%Y-%m-%d %H:%M:%S')
...
def appHomePageData(request):
    adData = adManageJsonSelectFn()
    goodsData = getShangjiaGoodsListFn()
    recommendData = getRecommendGoodsFn()
    temp_data = {
        'adData': adData , 
        'goodsData': goodsData , 
...
#     uwsgi.websocket_handshake()   #与客户端建立连接 ，在uwsgi2.0中，这个函数不需要参数
#     # myBigSocket = uwsgi
#     xiaoxi_client[0] = uwsgi
#     uwsgi.websocket_send('你好, 很高兴为您服务')

#     time.sleep(10)
#     uwsgi.websocket_send('你好, 我睡醒了')
...
# 根据商品id获取轮播图数组
def getLunboImagesByGoodsid(goodsid):
...
def goodsDataRowToDic(row):
    goods = {
        ...
        'status':row[15],
        'uptime':row[16].strftime('%Y-%m-%d %H:%M:%S'),
        'downtime':row[17].strftime('%Y-%m-%d %H:%M:%S'),
        'transportmoney':row[18],
        'addtime':row[19].strftime('%Y-%m-%d %H:%M:%S') , 
        'sellcount':row[20] , 
        'isinfudai':row[21] , 
        'isinmiaosha':row[22] , 
        'keywords':row[23] , 
        ...
        'xiaoClassiData':getAllFenleiInfoByMinfenleiId(row[34]) , 
        'haoping': row[35] , 
        'yiban': row[36] , 
        'chaping': row[37],
        'is_temai': row[38],
    ...
    if goods["isinmiaosha"] == "true":
        ...
        for ms in miaosha_info:
            ...
            tempDic["starttime"] = ms[2].strftime('%Y-%m-%d %H:%M:%S')
            tempDic["stoptime"] = ms[3].strftime('%Y-%m-%d %H:%M:%S')
            tempDic["miaoShaCount"] = ms[4]
            tempDic["alreadyMiaoShaNumber"] = ms[5]
            goods["miaosha_info"] = tempDic
    

    lunboArr = getLunboImagesByGoodsid(row[0])
...
# 根据小分类id获取整个分类信息
def getAllFenleiInfoByMinfenleiId(minFenleiId):
    cursor = connection.cursor()
    sqlStr = "select * from minfenlei where id='%s'" % minFenleiId
    cursor.execute(sqlStr)
    rows = cursor.fetchall()
    tempDic = {}
    for row in rows:
        ...
        tempDic["image"] = row[4]
        cursor.execute("select fatherid , name from midfenlei where id='%s'" % row[1])
        midRows = cursor.fetchall()
        for midRow in midRows:
            tempDic["midId"] = midRow[0]
            tempDic["midName"] = midRow[1]
            cursor.execute("select name , id from bigfenlei where id='%s'" % midRow[0])
            bigRows = cursor.fetchall()
            for bigRow in bigRows:
                tempDic["bigName"] = bigRow[0]
                tempDic["bigId"] = bigRow[1]
...
def notInMiaoshaGoods(request):
    try:
        ...
        if int(page) <= 1:
            page = 1
        start = (int(page) - 1) * 10
        sqlStr = "select * from goods where isinmiaosha='false' limit %d , 10" % start
        cursor = connection.cursor()
        cursor.execute(sqlStr)
        rows = cursor.fetchall()
        ...
        cursor.execute("SELECT COUNT(*) FROM goods where isinmiaosha='false'")
...
# 测试的注册接口  给李航用的 要删掉的
def loginRegiste(request):
    userName = request.POST.get("username" , "")
    password = request.POST.get("password" , "")

    cursor = connection.cursor()
    sqlStr = "insert into manager (username , pwd) values ('%s' , '%s')" % (userName , password)
    cursor.execute(sqlStr)

    cursor.close()

    successMSG = MyTool.resultOk("注册成功");
...
def getUserDetailByWxid(wxid):
    ...
    for row in rows:
        oneData = {
            ...
            "bankcard":row[10] , 
            "power":row[11] , 
            "registertime":row[12].strftime('%Y-%m-%d %H:%M:%S') , 
            "qrcode":row[13] , 
            "country":row[14] , 
            "gender":row[15] , 
            "city":row[16] , 
...
def getLunboList(request):
    ...
    lunboArr = getLunboImagesByGoodsid(goodsid)
...
def addLunboImages(request) :
    try:
        ...
        for key in request.FILES:
            ...
            cursor.execute(sqlStr)
            cursor.close()
            resultData["data"].append("http://www.dangsh.top/static/myfile/uploadImage/" + imgName)
...
def rebateTableQuery(request):
    try:
        ...
        for row in rows:
            ...
            wxid = row[1]
            goodsid = row[2]
            rebatetime = row[3].strftime('%Y-%m-%d %H:%M:%S')
            rebateMoney = row[4]
...
def getShangjiaGoodsListFn():
    cursor = connection.cursor()
    # sqlStr = "select * from goods where status = '1' limit %d , 20" % int(startNumber)
    ...
    try:
        ...
        for row in cursor.fetchall():
            ordertable = {
                ...
                'status':row[3],
                'goodsInfo':[] , 
                'createTime':row[2].strftime('%Y-%m-%d %H:%M:%S'),
                'freightPrice':row[4],
...
def favoriteAndLookTableManageJsonSelectFn(wxid , tablename):
    ...
    for data in cursor.fetchall():
        tempDic = {
            ...
            'id':data[1] , 
            'goods':{} , 
            'addtime':data[3].strftime('%Y-%m-%d %H:%M:%S')
        }
...
def deleLeaveMessage(request):
    ...
    wxid = data["wxid"]
    cursor = connection.cursor()
    sql = " delete from message where goodsid = '"+goodsid+"' and wxid = '" + wxid +"'"
    result = cursor.execute(sql)
    cursor.close() 
    if result == 0:
...
def findShare(request):
    try:
        ...
        cursor.close()
        for data in datas:
            # "sharetime":data[4].strftime('%Y-%m-%d %H:%M:%S')
            tempDic = {
                "shareid":data[0] , 
                "wxid":data[2] , 
                "sharepath":data[3] , 
    ...
    try:
        ...
        for data in cursor.fetchall():
            tempDic = {
                ...
                'buyid':data[1] , 
                'number':data[3] , 
                'buytime':data[4].strftime('%Y-%m-%d %H:%M:%S') , 
                'params':data[5] , 
                'goods':{}
            }
            tempDic["goods"] = getGoodsDetailByGoodsid(data[2])
...
def liuyan_and_dingdan(request):  
    ...
    # 留言列表查询
    cursor.execute("SELECT * FROM guestbook order by leavtime")
    data = cursor.fetchall()
    for row in data:
        tempDic = {}
        tempDic["userinfo"] = getUserDetailByWxid(row[1])
        tempDic["guestbookid"] = row[0]
        tempDic["leavemessage"] = row[2]
        tempDic["leavtime"] = row[3].strftime('%Y-%m-%d %H:%M:%S')
        tempDic["status"] = row[4]
        tempDic["direction"] = row[5]
        dataArr["liuyan_list"].append(tempDic)
...
def read_all_msg_by_wxid(request):
    try:
        ...
        if wxid == "":
            return HttpResponse(json.dumps({"message":"参数错误" , "status":"error"}) , content_type="application/json")
        cursor = connection.cursor()
        result = cursor.execute("update guestbook set status = 'read' where wxid = '%s'" % wxid)
        if result >= 1:
            return HttpResponse(json.dumps({"message":"操作成功" , "status":"ok"}) , content_type="application/json")
        else :
            return HttpResponse(json.dumps({"message":"操作失败" , "status":"error"}) , content_type="application/json")
    except Exception as e :
        return HttpResponse(json.dumps({"message":"操作异常，请联系服务器管理人员" , "status":"error"}) , content_type="application/json")
    ...
    try:
        ...
        for data in cursor.fetchall():
            tempDic = {
                ...
                'goods':{} , 
                'startTime':data[2].strftime('%Y-%m-%d %H:%M:%S') , 
                'stopTime':data[3].strftime('%Y-%m-%d %H:%M:%S')  , 
                'miaoShaCount':data[4] , 
        ...
        return HttpResponse(json.dumps({"data":myData , "message":"秒杀接口查询异常,请联系服务器人员" , "status":"error"}) , content_type="application/json");
...
def leavingMessAdd(request):
    ...
    if wxid == "" or status == "" or leavemessage == "" or direction == "":
        return HttpResponse(json.dumps({'message': '参数错误','status':'error'}), content_type="application/json")
    
    guestbookid = randomString()
    cursor=connection.cursor();
    result = cursor.execute("INSERT INTO guestbook (guestbookid , wxid , leavemessage , status , direction) VALUES ('%s' , '%s' , '%s' , '%s' , '%s')"%(guestbookid , wxid , leavemessage , status , direction))
    cursor.execute("select * from guestbook where guestbookid='%s'" % guestbookid)
    data = ''
    for row in cursor.fetchall():
        data = row[3].strftime('%Y-%m-%d %H:%M:%S')
    cursor.close();
    if result == 1:
        return HttpResponse(json.dumps({'message': '留言成功','status':'ok' , 'time':data}), content_type="application/json");
    else:
...
def selectAllleav(data):
    ...
    for ss in data:
        xiaoSB.append({
            ...
            "wxid":ss[1] , 
            "leavemessage":ss[2] , 
            "leavetime":ss[3].strftime('%Y-%m-%d %H:%M:%S') , 
            "status":ss[4] , 
            "direction":ss[5]
        })
    return(xiaoSB)
...
def leaveMessage(request):
    ...
    imgDic = []
    for i in a:
        aaa = {"goodsid":i[0] , "wxid":i[1] , "leavemessage":i[2]}
        imgDic.append(aaa)
...
def addLeaveMessage(request):
    ...
    # wxid = "2017121519245103"
    # leavemessage = "啥的感觉爱仕达将刷机大师"
    cursor = connection.cursor()
    result = cursor.execute("INSERT INTO message(goodsid , wxid , leavemessage)VALUES('%s' , '%s' , '%s')"%(goodsid , wxid , leavemessage))
    cursor.close()
    if result == 1:
...
def getWxPayidFn(pass_data):
    ...
    notify_url = 'http://www.dangsh.top/weixinNotify_url/'
    openid = pass_data['wxid']
    out_trade_no = pass_data['orderid']
    sign_type = 'MD5'
    spbill_create_ip = '39.106.19.54'
    total_fee = str(int(float(pass_data['totalMoney']) * 100))
    trade_type = 'JSAPI'
    key = 'liurundong0000000000000000000000'
...
def getSanjifenleiData(request):
    ...
    for row in rows:
        tempDic = {
            ...
            'fatherid':row[1] , 
            'name':row[2] , 
            'addtime':row[3].strftime('%Y-%m-%d %H:%M:%S') , 
            'image':row[4]
        }
...
def getAllFenleiByDafenleiId(request):
    ...
    for erjiRow in erjiRows:
        erjiTempDic = {
            ...
            "fatherid":erjiRow[1] , 
            "name":erjiRow[2] , 
            "addtime":erjiRow[3].strftime('%Y-%m-%d %H:%M:%S') , 
            "data":[]
        }
        ...
        for sanjiRow in sanjiRows:
            sanjiTempDic = {
                ...
                "fatherid":sanjiRow[1] , 
                "name":sanjiRow[2] , 
                "addtime":sanjiRow[3].strftime('%Y-%m-%d %H:%M:%S') , 
                "image":sanjiRow[4]
            }
...
def allBigFenlei(request):
    ...
    for row in rows:
        tempDic = {
            'id':row[0] , 
            'name':row[1] , 
            'addtime':row[2].strftime('%Y-%m-%d %H:%M:%S')
        }
